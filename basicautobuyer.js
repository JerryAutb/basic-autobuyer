...
(function () {
  ...
  var nameFilterDropdown = "#elem_" + makeid(15),
    ...
    nameNotificationType = "#elem_" + makeid(15),
    nameDiscordToken = "#elem_" + makeid(15),
    nameDiscordChannelId = "#elem_" + makeid(15),
  ...
  window.deleteFilter = function () {
    ...
    if (filterName != "Choose filter to load") {
      ...
      Object.assign(
        window.controllerInstance._viewmodel.searchCriteria,
        window.controllerInstance._viewmodel.defaultSearchCriteria
      );
  ...
  window.loadFilterByName = function (filterName) {
    ...
    if (settingsJson.abSettings.telegramBotToken) {
      jQuery(nameTelegramBotToken).val(
        settingsJson.abSettings.telegramBotToken
      );
    }
    ...
    if (settingsJson.abSettings.telegramBuy) {
      jQuery(nameNotificationType).val(settingsJson.abSettings.telegramBuy);
    }
    if (settingsJson.abSettings.discordToken) {
      jQuery(nameDiscordToken).val(settingsJson.abSettings.discordToken);
    }
    if (settingsJson.abSettings.discordChannelId) {
  ...
  window.sendNotificationToUser = function (message) {
    if (window.notificationEnabled) {
      ...
      let channelId = jQuery(nameDiscordChannelId).val();
      if (channelId) {
        if (window.discordClient) {
          window.discordClient.channels.get(channelId).send(message);
        } else {
          window.discordClient = window.initializeDiscordClient(function () {
            setTimeout(function () {
              if (window.discordClient)
                window.discordClient.channels.get(channelId).send(message);
            }, 200);
  ...
  window.activateAutoBuyer = function (isStart) {
    ...
    if (!window.discordClient)
      window.discordClient = window.initializeDiscordClient();
    ...
    services.Item.requestWatchedItems().observe(this, function (t, response) {
      ...
      if (isStart) {
        ...
        if (window.userWatchItems.length) {
          writeToDebugLog(
            `Found ${window.userWatchItems.length} items in users watch list and ignored from selling`
          );
        }
  ...
  window.UTAutoBuyerViewController.prototype.init = function init() {
    if (!this.initialized) {
      ...
      this._viewmodel ||
        ...
        this._viewmodel.searchCriteria.type === SearchType.ANY &&
          (this._viewmodel.searchCriteria.type = SearchType.PLAYER);
      ...
      var t = getAppMain()
        ...
        count =
          1 +
          (JSUtils.isValid(t)
            ? t[EAConfigurationRepository.KEY_ITEMS_PER_PAGE]
            : 15);
      ...
      view.addTarget(
        ...
        this._viewmodel.getCategoryTabVisible() &&
          (view.initTabMenuComponent(),
          view
            .getTabMenuComponent()
            .addTarget(this, this._eSearchCategoryChanged, EventType.TAP)),
        this._squadContext
          ? isPhone() || view.addClass("narrow")
    ...
    if (
      ...
      getAppMain().getRootViewController().showGameView =
        function showGameView() {
          ...
          if (
            (s.initWithRootController(new UTHomeHubViewController()),
            ...
            ST.setText(
              services.Localization.localize("navbar.label.customizeHub")
            ),
            ...
            I.init(),
              ...
              I.setText(
                services.Localization.localize("nav.label.leaderboards")
              ),
  ...
  window.findMinBin = async function () {
    ...
    if (minBuy) {
      window.notify(
        "Succesfully computed the price",
        enums.UINotificationType.POSITIVE
      );
    } else {
      window.notify(
    ...
    while (!isMinFound) {
      if (++currentCount === 10) {
        ...
        if (items.length) {
          ...
          let currentCalcBin = window.fixRandomPrice(
            window.getSellBidPrice(currentMin)
          );

          if (currentCalcBin === criteria.maxBuy) {
            isMinFound = true;
          } else {
            criteria.maxBuy = window.fixRandomPrice(
              window.getSellBidPrice(currentMin)
            );
          }
  ...
  window.createAutoBuyerInterface = function () {
    ...
    if (
      ...
      getAppMain()
        ...
        .getPresentedViewController()
        .getCurrentViewController()
        .getCurrentController()._jsClassName
    ) {
      if (!jQuery(".SearchWrapper").length) {
        ...
        jQuery(view.__root.parentElement).prepend(
          "<style>" +
            ...
            '                   <div id="' +
            nameAbSearchProgress.substring(1) +
            ...
            '               <div style="float: right; height: 10px; width: 100px; background: #888; margin: 5px 0px 5px 5px;">' +
            '                   <div id="' +
            nameAbStatisticsProgress.substring(1) +
            '" style="background: #000; height: 10px; width: 0%"></div>' +
            "               </div>" +
            "           </div>" +
            "       </div>" +
            ...
            nameAbCoins.substring(1) +
            '"></div>' +
            "   </div>" +
            '   <div class="view-navbar-clubinfo">' +
            '       <div class="view-navbar-clubinfo-data">' +
            '           <span class="view-navbar-clubinfo-name">Sold Items: <span id="' +
            nameAbSoldItems.substring(1) +
            '"></span></span>' +
            '           <span class="view-navbar-clubinfo-name">Unsold Items: <span id="' +
            nameAbUnsoldItems.substring(1) +
            '"></span></span>' +
            "       </div>" +
            "   </div>" +
            '   <div class="view-navbar-clubinfo" style="border: none;">' +
            '       <div class="view-navbar-clubinfo-data">' +
            '           <span class="view-navbar-clubinfo-name">Available Items: <span id="' +
            nameAbAvailableItems.substring(1) +
            '"></span></span>' +
            '           <span class="view-navbar-clubinfo-name">Active transfers: <span id="' +
            nameAbActiveTransfers.substring(1) +
            '"></span></span>' +
            "       </div>" +
            "   </div>" +
            "</div>"
        ...
        jQuery(view.__root.parentElement).append(
          '<div id="' +
            ...
            '" style="font-size: 10px; width: 100%;height: 37%;background-color:#141414;color:#e2dde2;"></textarea></div>'
        );
          ...
          jQuery(".ut-item-search-view")
            ...
            .prepend(
              '<div style="width:100%;display: flex;' +
                ...
                '<button style="width:100%" class="btn-standard call-to-action" id="' +
                namePreserveChanges.substring(1) +
                '">Save Filter</button>' +
                "</div> </div>"
            );
          jQuery(".search-prices")
            ...
            .append(
              "<div><br></div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbBuyPrice.substring(1) +
                '" placeholder="5000">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameAbCardCount.substring(1) +
                '" placeholder="10">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameAbItemExpiring.substring(1) +
                '" placeholder="1H">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                '<div class="price-filter">' +
                '<div class="button-container">' +
                '    <button class="btn-standard call-to-action" id="' +
                nameCalcBinPrice.substring(1) +
                '">Calculate Buy Price</button>' +
                "</div>" +
                "</div>" +
                "<div><br></div>" +
                "<hr>" +
                ...
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Sell Price:</span><br/><small>Receive After Tax: <span id="' +
                nameSellAfterTax.substring(1) +
                '">0</span></small>' +
                "   </div>" +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbSellPrice.substring(1) +
                '" placeholder="7000">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbMinDeleteCount.substring(1) +
                '" placeholder="10">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameAbMaxPurchases.substring(1) +
                '" placeholder="3">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbRandMinBidInput.substring(1) +
                '" placeholder="300">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '           <div id="' +
                nameAbRandMinBidToggle.substring(1) +
                '" class="ut-toggle-control">' +
                '           <div class="ut-toggle-control--track">' +
                "           </div>" +
                '           <div class= "ut-toggle-control--grip" >' +
                ...
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbRandMinBuyInput.substring(1) +
                '" placeholder="300">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '           <div id="' +
                nameAbRandMinBuyToggle.substring(1) +
                '" class="ut-toggle-control">' +
                '           <div class="ut-toggle-control--track">' +
                "           </div>" +
                '           <div class= "ut-toggle-control--grip" >' +
                ...
                '           <div id="' +
                nameAbCloseTabToggle.substring(1) +
                '" class="ut-toggle-control">' +
                '           <div class="ut-toggle-control--track">' +
                "           </div>" +
                '           <div class= "ut-toggle-control--grip" >' +
                ...
                '           <div id="' +
                nameAbSolveCaptcha.substring(1) +
                '" class="ut-toggle-control">' +
                '           <div class="ut-toggle-control--track">' +
                "           </div>" +
                '           <div class= "ut-toggle-control--grip" >' +
                ...
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameTelegramBotToken.substring(1) +
                '" style="text-transform: none;">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameTelegramChatId.substring(1) +
                '" style="text-transform: none;">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Discord Bot Token<br/><small>Your Discord Bot Token</small>:</span>' +
                "   </div>" +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameDiscordToken.substring(1) +
                '" style="text-transform: none;">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Discord Channel ID<br/><small>Your Discord Channel ID</small>:</span>' +
                "   </div>" +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameDiscordChannelId.substring(1) +
                '" style="text-transform: none;">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Notification Type<br/><small>Type A for buy/loss notification, B for buy only or L for lost notification only </small>:</span>' +
                "   </div>" +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <input type="text" class="numericInput" id="' +
                nameNotificationType.substring(1) +
                '" style="text-transform: none;">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '           <div id="' +
                nameAbMessageNotificationToggle.substring(1) +
                '" class="ut-toggle-control">' +
                '           <div class="ut-toggle-control--track">' +
                "           </div>" +
                '           <div class= "ut-toggle-control--grip" >' +
                ...
                '<div class="button-container">' +
                '    <button class="btn-standard call-to-action" id="' +
                nameTestNotification.substring(1) +
                ...
                "</div>" +
                '<div class="button-container">' +
                '       <select multiple="multiple" class="multiselect-filter" id="' +
                nameSelectedFilter.substring(1) +
                '" name="selectedFilters" style="padding: 10px;width: 100%;font-family: UltimateTeamCondensed,sans-serif;font-size: 1.6em;color: #e2dde2;text-transform: uppercase;background-color: #171826; overflow-y : scroll"></select>' +
                '       <label style="white-space: nowrap;" id="' +
                nameSelectFilterCount.substring(1) +
                ...
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbNumberFilterSearch.substring(1) +
                '" placeholder="1">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
                ...
                '       <div class="inputBox">' +
                '           <input type="tel" class="numericInput" id="' +
                nameAbStopErrorCode.substring(1) +
                '" placeholder="">' +
                "       </div>" +
                "   </div>" +
                "</div>" +
      ...
      if (!jQuery(nameSearchCancelButton).length) {
        jQuery(nameInfoWrapper)
          .next()
          .find(".button-container button")
          .last()
          .after(
            '<button class="btn-standard" id="' +
              nameSearchCancelButton.substring(1) +
              '">Stop</button><button class="btn-standard" id="' +
              nameClearLogButton.substring(1) +
              '">Clear Log</button>'
          );
      }
    } else {
  ...
  window.saveDetails = function () {
    ...
    setTimeout(function () {
      ...
      if (jQuery(nameAbMinDeleteCount).val() !== "") {
        settingsJson.abSettings.minDeleteCount =
      ...
      if (jQuery(nameAbRandMinBidInput).val() !== "") {
        settingsJson.abSettings.randMinBid = jQuery(
          nameAbRandMinBidInput
        ).val();
      }

      if (jQuery(nameAbRandMinBuyInput).val() !== "") {
        settingsJson.abSettings.randMinBuy = jQuery(
          nameAbRandMinBuyInput
        ).val();
      }
      ...
      if (jQuery(nameTelegramChatId).val() !== "") {
        settingsJson.abSettings.telegramChatID =
          jQuery(nameTelegramChatId).val();
      }
      if (jQuery(nameNotificationType).val() !== "") {
        settingsJson.abSettings.telegramBuy =
          jQuery(nameNotificationType).val();
      }
      if (jQuery(nameDiscordToken).val() !== "") {
        settingsJson.abSettings.discordToken = jQuery(nameDiscordToken).val();
      }
      if (jQuery(nameDiscordChannelId).val() !== "") {
        settingsJson.abSettings.discordChannelId =
          jQuery(nameDiscordChannelId).val();
      }

      if (window.notificationEnabled) {
        settingsJson.abSettings.notificationEnabled =
      ...
      if (filterName) {
        ...
        $(`select[name=filters] option[value="${filterName}"]`).attr(
          "selected",
          true
        );
  ...
  window.setFilters = function () {
    ...
    if (window.selectedFilters.length) {
      jQuery(nameSelectFilterCount).text(
        "(" + window.selectedFilters.length + ") Filter Selected"
      );
    } else {
  ...
  window.toggleRelist = function () {
    if (window.reListEnabled) {
      ...
      alert(
        "Re-listing will list all the cards in the transfer list not only the card which bought by the tool. " +
  ...
  jQuery(document).on("keyup", nameAbSellPrice, function () {
    jQuery(nameSellAfterTax).html(
      (
        jQuery(nameAbSellPrice).val() -
        (parseInt(jQuery(nameAbSellPrice).val()) / 100) * 5
      ).toLocaleString()
    );
  });
  ...
  window.getTimerProgress = function (timer) {
    ...
    return (
      (Math.max(0, timer.finish - time) / (timer.finish - timer.start)) * 100
    );
  };

  window.updateStatistics = function () {
    jQuery(nameAbSearchProgress).css(
      "width",
      window.getTimerProgress(window.timers.search)
    );
    jQuery(nameAbStatisticsProgress).css(
      "width",
  ...
  window.processor = window.setInterval(function () {
    if (window.autoBuyerActive) {
      ...
      if (
        ...
        (window.timers.search.finish == 0 ||
          window.timers.search.finish <= time)
      ) {
        let searchRequest = 1;
        ...
        window.timers.search = window.createTimeout(
          time,
      ...
      if (
        window.timers.coins.finish == 0 ||
        window.timers.coins.finish <= time
      ) {
        window.futStatistics.coins =
          services.User.getUser().coins.amount.toLocaleString();
        window.futStatistics.coinsNumber = services.User.getUser().coins.amount;
        window.timers.coins = window.createTimeout(time, 2500);
      }

      if (
        window.timers.transferList.finish == 0 ||
      ...
      if (
        !window.selectedFilters.length &&
        (window.timers.bidCheck.finish == 0 ||
          window.timers.bidCheck.finish <= time)
      ) {
  ...
  window.stopIfRequired = function () {
    ...
    if (time) {
      ...
      let timeElapsed =
        (currentTime.getTime() - window.botStartTime.getTime()) / 1000;
    ...
    if (
      window.buyCardCount &&
      window.purchasedCardCount >= window.buyCardCount
    ) {
      window.deactivateAutoBuyer(true);
    }
  };
  ...
  window.pauseIfRequired = function () {
    if (window.searchCountBeforePause <= 0) {
      ...
      time = Math.round(
        Math.random() * (waitTime[1] - waitTime[0]) + waitTime[0]
      );
  ...
  window.searchFutMarket = function (sender, event, data) {
    ...
    if (
      ...
      let filterName =
        window.selectedFilters[
          window.getRandNum(0, window.selectedFilters.length - 1)
        ];
    ...
    if (!window.eachFilterSearch) {
      if (jQuery(nameAbNumberFilterSearch).val() !== "") {
        window.eachFilterSearch = parseInt(
    ...
    if ($(nameAbItemExpiring).val() !== "") {
      ...
      let expiresInterval =
        expiresInString[expiresInString.length - 1].toUpperCase();
      let expiresInTime = parseInt(
        expiresInString.substring(0, expiresInString.length - 1)
      );

      let multipler =
        expiresInterval === "M" ? 60 : expiresInterval === "H" ? 3600 : 1;
      if (expiresInTime) {
    ...
    if (window.useRandMinBid) {
      ...
      searchCriteria.minBid = window.fixRandomPrice(
        window.getRandNum(0, user_min_bid)
      );
      window.currentPage = 1;
    }
    if (window.useRandMinBuy) {
      ...
      searchCriteria.minBuy = window.fixRandomPrice(
        window.getRandNum(0, user_min_buy)
      );
      window.currentPage = 1;
    }
    ...
    services.Item.searchTransferMarket(
      ...
      if (response.success && window.autoBuyerActive) {
        ...
        for (let i = 0; i < response.data.items.length; i++) {
          ...
          let priceToBid = window.bidExact
            ? bidPrice
            : isBid
            ? window.getSellBidPrice(bidPrice)
            : bidPrice;
          let checkPrice = window.bidExact
            ? priceToBid
            : isBid
            ? window.getBuyBidPrice(currentBid)
            : currentBid;
          ...
          let bid_buy_txt =
            "(bid: " +
            window.format_string(currentBid.toString(), 6) +
            " / buy:" +
            window.format_string(buyNowPrice.toString(), 7) +
            ")";
          ...
          if (
            player_rating >= selected_min_rate &&
            player_rating <= selected_max_rate
          ) {
            rating_ok = true;
            rating_ok_txt = "ok";
          }
          ...
          if (
            rating_ok &&
            window.autoBuyerActive &&
            buyNowPrice <= userBuyNowPrice &&
            buyNowPrice <= window.futStatistics.coinsNumber &&
            !window.bids.includes(auction.tradeId)
          ) {
        ...
        if (
          ...
          (response.error &&
            response.error.code == UtasErrorCode.CAPTCHA_REQUIRED)
        ) {
          if (window.autoSolveCaptcha) {
            writeToLog(
              "------------------------------------------------------------------------------------------"
              ...
              if (window.autoBuyerActive && bidPrice) {
                ...
                for (var i = 0; i < outBidItems.length; i++) {
                  ...
                  if (
                    ...
                    writeToDebugLog(
                      "Bidding on outbidded item -> Bidding Price :" +
                        checkPrice
                    );
                    buyPlayer(player, checkPrice, sellPrice);
                    window.addDelayAfterBuy && window.waitSync(1);
              ...
              if (window.autoBuyerActive && sellPrice && !isNaN(sellPrice)) {
                let boughtItems = response.data.items.filter(function (item) {
                  return (
                    ...
                    !window.userWatchItems.includes(item._auction.tradeId) &&
                    !window.sellBids.includes(item._auction.tradeId)
                  );
                });
                ...
                for (var i = 0; i < boughtItems.length; i++) {
                  ...
                  window.sellRequestTimeout = window.setTimeout(function () {
                    services.Item.list(
                      player,
                      window.getSellBidPrice(sellPrice),
                      sellPrice,
                      3600
                    );
                  }, window.getRandomWait());
  ...
  window.buyPlayer = function (player, price, sellPrice, isBin) {
    services.Item.bid(player, price).observe(this, function (sender, data) {
      ...
      if (data.success) {
        ...
        if (isBin && sellPrice !== 0 && !isNaN(sellPrice)) {
          ...
          writeToLog(
            sym +
              ...
              price_txt +
              (isBin
                ? " | buy | success | selling for: " + sellPrice
                : " | bid | success |" + " selling for: " + sellPrice)
          );
          ...
          services.Item.move(player, ItemPile.CLUB).observe(
            this,
            function (sender, moveResponse) {
              let sym =
                " B:" + window.format_string(window.bidCount.toString(), 4);
              writeToLog(
                sym +
                  " | " +
                  player_name +
                  ...
                  price_txt +
                  (isBin
                    ? " | buy | success | move to club"
                    : " | bid | success | waiting to expire")
              );
            }
          );
        }
        ...
        if (
          jQuery(nameNotificationType).val() == "B" ||
          jQuery(nameNotificationType).val() == "A"
        ) {
          window.sendNotificationToUser(
            "| " + player_name.trim() + " | " + price_txt.trim() + " | buy |"
          );
        }
      } else {
        window.lossCount++;
        ...
        writeToLog(
          sym +
            ...
            " | " +
            price_txt +
            (isBin ? " | buy | failure |" : " | bid | failure |") +
            " ERR: " +
            data.status +
            "-" +
            (errorCodeLookUpShort[data.status] || "")
        );
        if (
          jQuery(nameNotificationType).val() == "L" ||
          jQuery(nameNotificationType).val() == "A"
        ) {
          window.sendNotificationToUser(
            "| " +
        ...
        if (jQuery(nameAbStopErrorCode).val()) {
          ...
          if (errorCodes.indexOf(data.status + "") != -1) {
            ...
            writeToLog(
              `[!!!] Autostopping bot since error code ${data.status} has occured`
            );
            writeToLog(
              "------------------------------------------------------------------------------------------"
            );
  ...
  window.updateTransferList = function () {
    ...
    services.Item.requestTransferItems().observe(this, function (t, response) {
      ...
      window.futStatistics.unsoldItems = response.data.items.filter(function (
        item
      ) {
        return (
      ...
      window.futStatistics.activeTransfers = response.data.items.filter(
        function (item) {
      ...
      window.futStatistics.availableItems = response.data.items.filter(
        function (item) {
      ...
      if (window.futStatistics.soldItems >= minSoldCount) {
        ...
        writeToLog(
          "[TRANSFER-LIST] > " +
            window.futStatistics.soldItems +
            " item(s) sold"
        );
        writeToLog(
          "------------------------------------------------------------------------------------------"
  ...
  window.initializeDiscordClient = function (cb) {
    const client = new Discord.Client();
    let discordToken = jQuery(nameDiscordToken).val();
